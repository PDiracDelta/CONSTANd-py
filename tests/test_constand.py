...
class TestConstand(TestCase):
	def setUp(self):
		self.path_in = '/home/pdiracdelta/Documents/KUL/Master of Bioinformatics/Thesis/data/MB_noapostrophes.tsv'  # TEST
		self.delim_in = '\t'
		self.header_in = None
		self.accuracy = 1e-2
	...
	def test_constand_result(self):
		correctResult = np.asarray(importDataFrame(path_in=self.path_out, header=self.header_in))
