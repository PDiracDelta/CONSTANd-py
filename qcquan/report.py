	...
	for cond in schema['allConditions']:
		for eName in schema['allMSRuns']:
			if cond in schema[eName]:
				...
				colorsOfChannelAliases = np.tile(distColours[c], (numChannels, 1))
				if hex:
					colorsOfChannelAliases = np.array(unnest(colorsOfChannelAliases))
				channelColoursDict.update(dict(zip(schema[eName][cond]['channelAliases'], colorsOfChannelAliases)))
		c += 1
...
def getHCDendrogram(HCResult, schema, title=None):
	...
	channelColorsDict = getColours(schema, hex=True)
...
def makeHTML(jobParams, allProcessingParams, otherConditions, minTopDifferentialsDFs, fullTopDifferentialsDFs, minVolcanoFullPaths,
	...
	# remove 'modifications' columns
	for condition in otherConditions:
		if 'modifications' in minTopDifferentialsDFs[condition].columns:
			minTopDifferentialsDFs[condition] = minTopDifferentialsDFs[condition].drop('modifications', axis=1, inplace=False)
		if jobParams['fullExpression_bool']:
			if 'modifications' in fullTopDifferentialsDFs[condition].columns:
				fullTopDifferentialsDFs[condition] = fullTopDifferentialsDFs[condition].drop('modifications', axis=1, inplace=False)
